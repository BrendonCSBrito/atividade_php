...
if (isset($_GET['id'])) {
    $cod_cliente = (int)$_GET['id'];
    $sql = $pdo->prepare("SELECT * FROM tab_clientes WHERE id = $cod_cliente");
    $sql->execute();
...
if (isset($_POST['nome'])) {
    $sql = $pdo->prepare("UPDATE tab_clientes SET nome = ?, cpf = ?, email = ? WHERE id = $cod_cliente");
